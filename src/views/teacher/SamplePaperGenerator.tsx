...
export default function SamplePaperGenerator({ userClass, onBack }: { userClass: string | null, onBack?: () => void }) {
  ...
  const handleGenerate = async () => {
    if (!desc) return;
    setLoading(true);
    const res = await getGeminiResponse(`Generate a sample practice paper for: ${desc}`, prompts.samplePaperGenerator, userClass, "gemini-3.1-flash-lite-preview");
    setResponse(res);
    setLoading(false);
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-[#57534e] text-[11px] font-bold uppercase tracking-widest">Design practice exams that mimic real boards.</p>
        </div>
      </div>

      <div className="bg-white p-6 rounded-xl border border-[#e9ecef] shadow-sm space-y-4">
        <div className="space-y-1">
          <label className="text-[10px] font-black uppercase tracking-widest text-[#57534e] pl-1">Board/Subject Details</label>
          <textarea 
            value={desc}
        ...
        >
          {loading ? <RotateCcw className="w-4 h-4 animate-spin text-[#f59e0b]" /> : <> <Send className="w-4 h-4 text-[#f59e0b]" /> Generate Sample Paper </>}
        </button>
      </div>
