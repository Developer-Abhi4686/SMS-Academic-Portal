...
export default function TestPaperGenerator({ userClass, onBack }: { userClass: string | null, onBack?: () => void }) {
  const [subject, setSubject] = useSessionState('test_subject', '');
  const [mathPart, setMathPart] = useSessionState<'Part I' | 'Part II'>('test_math_part', 'Part I');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await getGeminiResponse(`Generate a full test paper based on: ${fullDesc}`, prompts.testPaperGenerator, userClass);
      setResponse(res);
    } catch (error: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10">
      <header className="flex items-center gap-6">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-accent">Exam Designer</p>
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-primary">Terminal Assessments</h1>
        </div>
      </header>
      ...
      <div className="bg-surface border border-border-subtle p-8 rounded-[2.5rem] shadow-sm space-y-8">
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-bold uppercase tracking-widest text-muted pl-1">Structural Specifications</label>
          <textarea 
            value={desc}
            onChange={(e) => setDesc(e.target.value)}
            placeholder="Outline sections, marks distribution, or specific topics to include..." 
            rows={5}
