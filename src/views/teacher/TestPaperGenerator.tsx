...
export default function TestPaperGenerator({ userClass, onBack }: { userClass: string | null, onBack?: () => void }) {
  ...
  const handleGenerate = async () => {
    if (!desc) return;
    setLoading(true);
    const res = await getGeminiResponse(`Generate a full test paper based on: ${desc}`, prompts.testPaperGenerator, userClass, "gemini-3.1-pro-preview");
    setResponse(res);
    setLoading(false);
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-[#636e72] text-[11px] font-bold uppercase tracking-widest">Create comprehensive exam papers instantly.</p>
        </div>
      </div>

      <div className="bg-white p-6 rounded-xl border border-[#e9ecef] shadow-sm space-y-4">
        <div className="space-y-1">
          <label className="text-[10px] font-black uppercase tracking-widest text-[#636e72] pl-1">Paper Requirements</label>
          <textarea 
            value={desc}
        ...
        >
          {loading ? <RotateCcw className="w-4 h-4 animate-spin text-[#00b8d4]" /> : <> <Send className="w-4 h-4 text-[#00b8d4]" /> Generate Test Paper </>}
        </button>
      </div>
      ...
      {response && (
        ...
        >
          ...
          <div className="absolute top-4 right-4 flex gap-2">
            <SaveButton 
              type="test-paper" 
              title={`Test Paper: ${desc.slice(0, 30)}${desc.length > 30 ? '...' : ''}`} 
              content={response} 
            />
            <button 
